...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView, SafeAreaView, ActivityIndicator, Platform, KeyboardAvoidingView } from "react-native";
import { BottomSheet } from "@rneui/themed";
import { apiService, RecentTransfer, SwiftPayUser, UserProfile } from "../services/api";
...
const TransferToSwiftpay: React.FC = () => {
	const params = useLocalSearchParams<{ tag?: string; amount?: string }>();
	...
	const { user, displayLoader, hideLoader, verifyPin, getUserProfile } = useAuth();
	...
	const fetchRecentTransfers = async () => {
		try {
			...
			setToastMessage(error.response?.data?.error || "Failed to fetch recent transfers");
			setToastType("error");
		...
		if (tag.length > 0) {
			try {
				...
				if (response.status === "success") {
					...
					setRecipientName(response.data.user.first_name + " " + response.data.user.last_name);
				}
			} catch (error: any) {
	...
	const handleNext = async () => {
		try {
			...
			setToastMessage(error.response?.data?.error || "Failed to confirm transfer");
			setToastType("error");
			setToastVisible(true);
		} finally {
	...
	const handleConfirmPayment = async (pin: string) => {
		...
		try {
			...
			const response = await apiService.transferToSwiftpay({
				...
				description: remark || "Transfer",
				pin,
				source_link: transferSource === "send_to_africa" ? transferSource : null,
			});
			...
			<View style={styles.container}>
				<KAScrollView>
					...
					<View>
						...
						<View style={styles.inputContainer}>
							<View style={styles.tagInputContainer}>
								...
								<TextInput
									...
									placeholder="johndoe"
									value={swiftPayTag.startsWith("@") ? swiftPayTag.substring(1) : swiftPayTag}
									onChangeText={(text) => {
							...
							{!recipientName && !showSameTagError && <Button text="Confirm User" outlined softBg onPress={() => handleSwiftPayTagChange(swiftPayTag.startsWith("@") ? swiftPayTag : `@${swiftPayTag}`)} classNames="p-4 mb-8 -mt-1" />}

							{showSameTagError && (
								<Animated.Text entering={ZoomIn} exiting={ZoomOut} className="text-red text-base mb-6">
							...
							<TextInput style={styles.input} placeholder="10,000" value={amount} keyboardType="numeric" onChangeText={setAmount} />
							<Text style={styles.label}>Remark</Text>
							<TextInput style={styles.input} placeholder="Part payment" value={remark} onChangeText={setRemark} />
						</View>

						{/* Tab Switcher */}
						<View style={styles.recentContainer}>
							<View style={styles.tabContainer}>
								<TouchableOpacity style={[styles.tab, selectedTab === "Recent" && styles.activeTab]} onPress={() => setSelectedTab("Recent")}>
									<Text style={[styles.tabText, selectedTab === "Recent" && styles.activeTabText]}>Recent</Text>
								</TouchableOpacity>

								<TouchableOpacity style={[styles.tab, selectedTab === "Favourites" && styles.activeTab]} onPress={() => setSelectedTab("Favourites")}>
									<Text style={[styles.tabText, selectedTab === "Favourites" && styles.activeTabText]}>Favourites</Text>
								</TouchableOpacity>
							</View>
								...
								<ScrollView showsVerticalScrollIndicator={false} style={styles.tabContent}>
									{loading ? (
											...
											>
												<View style={styles.userInfo}>
													...
													<View>
														...
														<Text style={styles.userTag}>@{transfer.username}</Text>
													</View>
												</View>
											</TouchableOpacity>
										))
										...
										<View style={styles.noTransfersContainer}>
											<Text style={styles.noTransfersText}>No recent transfers</Text>
										</View>
									)}
									{recentTransfers?.length > 0 && (
										<TouchableOpacity style={styles.viewAllButton} onPress={() => router.push("/SwiftPayBeneficiaries")}>
						...
						<Button text="Next" onPress={handleNext} disabled={!swiftPayTag || !amount || isConfirming || showSameTagError} isLoading={isConfirming} loadingText="Processing..." />
					</View>
						...
						<View style={[styles.bottomSheetContent, { padding: 24 }]}>
							<View style={styles.bottomSheetHeader}>
								...
								<TouchableOpacity onPress={() => setIsPaymentSummaryVisible(false)}>
									<AntDesign name="closecircle" size={22} color={"#D32F2F"} style={styles.closeIcon} />
								</TouchableOpacity>
							</View>
							...
							<View style={styles.summaryContainer}>
								...
								<View style={styles.summaryItemContainer}>
									...
									<View style={styles.summaryItem}>
										...
										<Text style={styles.summaryValue}>{recipientName.length > 23 ? `${recipientName.slice(0, 23)}...` : recipientName}</Text>
									</View>
									...
									<View style={styles.summaryItem}>
										...
										<Text style={styles.summaryValue}>{remark || "Transfer"}</Text>
									</View>
								</View>
							</View>
							...
							<View style={styles.paymentMethodSection}>
								...
								<View style={styles.paymentMethodContainer}>
									<View style={styles.paymentMethodInfo}>
										...
										<Text style={styles.paymentMethodBalance}>₦{(userProfile?.wallet_balance || 0).toLocaleString()}</Text>
									</View>
					...
					>
						<View style={styles.bottomSheetContent}>
							...
							{error && <Text className="text-danger font-medium text-[16px] text-center mt-2">{error}</Text>}

							<PinComponent onComplete={(pin: string) => handleConfirmPayment(pin)} setModalState={setIsTransactionPinVisible} />
						</View>
					</BottomSheet>
						...
						<View style={[styles.bottomSheetContent, { padding: 24 }]}>
							...
							<View style={styles.successDetailsContainer}>
								<Text style={styles.successDescription}>
									Your transfer of <Text style={styles.successHighlight}>₦{parseFloat(transferAmount).toLocaleString()}</Text> to <Text style={styles.successHighlight}>{transferRecipient}</Text> has been completed successfully.
								</Text>
							</View>
