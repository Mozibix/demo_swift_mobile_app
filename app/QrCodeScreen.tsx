...
import { BarcodeScanningResult, CameraType, CameraView, useCameraPermissions, Camera } from "expo-camera";
...
import { ActivityIndicator, Linking, Modal, StyleSheet, Text, TouchableOpacity, Vibration, View } from "react-native";
...
const QrCodeScreen = () => {
		...
		try {
			...
			if (status !== "granted") {
				Toast.show({
					...
					text2: "We need permission to access your photos to scan QR codes from images.",
					visibilityTime: 3000,
				});
				return;
			}
			...
			if (!result.canceled && result.assets && result.assets.length > 0) {
				...
				try {
					...
					if (read_result) {
						const { tag, amount } = parseQRCode(read_result);
	...
	const parseQRCode = (data: string): { tag: string; amount?: string } => {
		// Check if QR code contains amount (format: @username|amount)
		if (data.includes("|")) {
			const [tag, amount] = data.split("|");
			return { tag, amount };
		}
		return { tag: data };
	};
	...
	const handleBarcodeScanned = useCallback(
		(scanningResult: BarcodeScanningResult) => {
			...
			try {
				...
				// Parse QR code data
				const { tag, amount } = parseQRCode(data);
	...
	if (!permission) {
		...
		return (
			<View style={[styles.container, styles.centered]}>
				...
				<Text style={{ color: "#fff", marginTop: 20 }}>Initializing camera...</Text>
			</View>
		);
	}
	...
	if (!permission.granted) {
		return (
			<View style={[styles.container, styles.centered]}>
				<Image source={require("../assets/camera.svg")} style={{ height: 200, width: 200 }} />
				<Text style={styles.errorText} className="mt-5 max-w-[90%] text-[16px] leading-7">
					<Text className="text-[19px]">Allow camera access in this next screen for:</Text>
					...
					You can change this option anytime later in the Settings app {!permission.canAskAgain && "To enable this feature, please allow camera access in your device settings under SwiftPay."}
				</Text>
	...
	return (
		<SafeAreaView style={styles.container}>
			...
			<Modal animationType="slide" transparent={true} visible={showMyQRCode} onRequestClose={() => setShowMyQRCode(false)}>
				<View style={styles.modalContainer}>
					<View style={styles.modalContent}>
						...
						<ViewShot ref={viewShotRef} options={{ format: "png", quality: 0.9 }} style={styles.shareableCard}>
							<LinearGradient colors={["#0047cc", "#0066ff", "#0080ff"]} style={styles.cardBackground} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
								<View style={styles.cardHeader}>
									<Image source={require("../assets/icons/icon.png")} style={styles.cardHeaderLogo} />
								...
								<View style={styles.qrCodeContainer}>
									{userProfile ? (
										<View style={styles.qrWrapper}>
											<SvgQRCode value={qrValue} size={220} backgroundColor="white" color="#0033cc" logo={require("../assets/icons/icon.png")} logoSize={40} logoBackgroundColor="white" logoMargin={5} logoBorderRadius={10} />
										</View>
									) : (
								...
								<Text style={styles.scanInstructions}>Scan to send money instantly</Text>

								<View style={styles.cardFooter}>
									<Image source={require("../assets/icons/icon.png")} style={styles.cardFooterLogo} resizeMode="contain" />
									<Text style={styles.cardFooterText}>Secure • Fast • Reliable</Text>
								</View>
							</LinearGradient>
						</ViewShot>
			...
			<Modal animationType="slide" transparent={true} visible={showScannedModal} onRequestClose={() => setShowScannedModal(false)}>
				<View style={styles.modalContainer}>
					<View style={styles.modalContent}>
						<View style={styles.modalHeader}>
						...
						<View style={styles.scannedResultContainer}>
							<MaterialCommunityIcons name="qrcode-scan" size={50} color={COLORS.swiftPayBlue} style={styles.scannedIcon} />
							...
							{scannedAmount && (
								<View style={styles.scannedAmountContainer}>
									<MaterialCommunityIcons name="cash" size={20} color={COLORS.swiftPayBlue} />
									<Text style={styles.scannedAmountText}>₦{scannedAmount}</Text>
								</View>
							)}

							<Text style={styles.scannedDescription}>{scannedAmount ? `You're about to pay ${scannedTag} the amount of ₦${scannedAmount}` : `You're about to make a payment to this SwiftPay tag`}</Text>
						</View>
						...
						<View style={styles.scannedActionButtons}>
							...
							<TouchableOpacity style={styles.continueButton} onPress={proceedToPayment}>
								<Text style={styles.continueButtonText} className="whitespace-nowrap">
									Continue to Payment
								</Text>
							</TouchableOpacity>
						</View>
						...
						<View style={styles.overlay}>
							...
							<View style={styles.middleContainer}>
								...
								<View style={styles.focusedContainer}>
									...
									<View style={[styles.scannerMarker, styles.bottomLeftMarker]} />
									<View style={[styles.scannerMarker, styles.bottomRightMarker]} />

									<View style={styles.scanFocusArea}>
										<Image source={require("../assets/icons/icon.png")} style={styles.logoWatermark} resizeMode="contain" />
									</View>
			...
			<View style={styles.footer}>
				...
				<View style={styles.controls}>
					<TouchableOpacity onPress={toggleFlash} style={styles.controlButton}>
						<MaterialIcons name={flashEnabled ? "flash-on" : "flash-off"} size={28} color={COLORS.swiftPayBlue} />
						<Text style={styles.controlText}>Flash</Text>
					</TouchableOpacity>

					<TouchableOpacity onPress={pickImage} style={styles.controlButton}>
						<MaterialIcons name="photo-library" size={28} color={COLORS.swiftPayBlue} />
						<Text style={styles.controlText}>Gallery</Text>
					</TouchableOpacity>

					<TouchableOpacity onPress={toggleCameraFacing} style={styles.controlButton}>
						<MaterialIcons name="flip-camera-android" size={28} color={COLORS.swiftPayBlue} />
						<Text style={styles.controlText}>Flip</Text>
					</TouchableOpacity>

					<TouchableOpacity onPress={toggleMyQRCode} style={styles.controlButton}>
						<MaterialCommunityIcons name="qrcode" size={28} color={COLORS.swiftPayBlue} />
						<Text style={styles.controlText}>My QR</Text>
					</TouchableOpacity>

					{scanned && !loading && !showScannedModal && (
						<TouchableOpacity onPress={() => setScanned(false)} style={[styles.scanAgainButton, styles.controlButton]}>
							<MaterialCommunityIcons name="qrcode-scan" size={28} color={COLORS.swiftPayBlue} />
